...
Upozornění: Funkčnost scriptu je ověřena na servru merlin, na jiných strojích se může
            postup spuštění a instalace lišit.

Základní myšlenkou testování je porovnání výsledku IFJ překladače a interpretru
s výsledkami stejného programu v jazyce go.
...
Použití:
Testy spustíte vykonáním příkazu "python2 ./testsuite.py".
...
koncovku .go a na začátku obsahovat testovací hlavičku, která obsahuje informace
...
"//interpret <interpret-codes>":
	...
	Jednotlivé číselné kódy jsou odděleny mezerou.
	Pokud seznam obsahuje nulu, následuje po úspěšné interpretaci porovnání s výsledkem go interpretu.
	Pokud tento řádek není přítomen, je nula jediný validní návratový kód.
"//nogo":
	Pokud je tento řádek přítomen, neprovádí se porovnání výsledků interpretace
	s výsledky go interpretu. Toto je vhodné pro situace, které jazyk ifj20
	zpracovává jinak než jazyk go (např. nepoužité proměnné).
"//extensions+ <extensions>":
	Seznam rozšíření, které je nutné implementovat pro korektní průběh daného testu.
	Rozšíření jsou uváděna v textová podobě (stejné jako v zadání projektu)
	a jsou oddělena mezerou. Pokud tento řádek není přítomen, není nutné
	mít implementováno žádné rozšíření pro korektní výsledek testu.
"//extensions- <extensions>":
	Seznam rozšíření, které nesmí být implementovány pro korektní průběh daného testu.
	Rozšíření jsou uváděna v textová podobě (stejné jako v zadání projektu)
	a jsou oddělena mezerou. Pokud tento řádek není přítomen, není zakázana
